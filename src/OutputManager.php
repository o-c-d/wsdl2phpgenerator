...
class OutputManager
{
    ...
    private function setOutputDirectory()
    {
        ...
        if (!empty($this->config->get('namespaceModelSuffix'))&&is_dir($outputDirectory.DIRECTORY_SEPARATOR.$this->config->get('namespaceModelSuffix')) == false) {
            if (mkdir($outputDirectory.DIRECTORY_SEPARATOR.$this->config->get('namespaceModelSuffix'), 0777, true) == false) {
                throw new Exception('Could not create output directory and it does not exist!');
            }
        }
    ...
    private function saveClassToFile(PhpClass $class, bool $isModel = false)
    {
        if ($this->isValidClass($class)) {
            ...
            if($isModel && !empty($this->config->get('namespaceModelSuffix'))) $namespace .= '\\'.$this->config->get('namespaceModelSuffix');
            if (!empty($namespace)) {
    ...
    private function saveAutoloader($name, PhpClass $service, array $types)
    {
        ...
        foreach ($types as $class) {
            $className = $this->config->get('namespaceName') .( !empty($this->config->get('namespaceModelSuffix')) ? '\\'.$this->config->get('namespaceModelSuffix'):'' ). '\\' . $class->getIdentifier();
            $className = ltrim($className, '\\');
            $autoloadedClasses[] = "'" . $className . "' => __DIR__ .'/" .( !empty($this->config->get('namespaceModelSuffix')) ? $this->config->get('namespaceModelSuffix').DIRECTORY_SEPARATOR:'' ). $class->getIdentifier() . ".php'";
        }
